...
/*
  1. rename cgic.c -> cgic.cpp
  2. replace WIN32 to _WIN32
  3. cgiRequestUri
  by sowicm.com
*/
...
int main() {
    ...
    cgiGetenv(&cgiRequestUri, "REQUEST_URI");
	...
	if (cgiStrEqNc(cgiRequestMethod, "post")) {
		...
		if (cgiStrEqNc(cgiContentType, "application/x-www-form-urlencoded")) {	
			...
			CGICDEBUGEND	
#endif /* CGICDEBUG */
            auto res = cgiParsePostMultipartInput();
			if (res != cgiParseSuccess) {
#ifdef CGICDEBUG
				CGICDEBUGSTART
...
cgiFormEntry *cgiFormEntryFindFirst(const char *name) {
	cgiFindTarget = name;
